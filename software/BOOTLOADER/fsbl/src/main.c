...
int main(void)
{
	...
	XAdcPs adcps;
	XAdcPs_Config* adccfg = XAdcPs_LookupConfig(0);


	Status = XAdcPs_CfgInitialize(&adcps, adccfg, XPAR_PS7_XADC_0_BASEADDR);
	if(Status != XST_SUCCESS) {
		fsbl_printf(DEBUG_INFO, "XADCPS INIT FAILED!\r\n");
		FsblHookFallback();
	}

	XAdcPs_Reset(&adcps);
	printf("Chip temperature: %f C\r\n", ((((XAdcPs_GetAdcData(&adcps, XADCPS_CH_TEMP)>>4) * 503.975) / 4096) - 273.15));
	printf("VCCINT: %f V\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VCCINT)>>4)/4096.0f*3.0f);
	printf("VCCAUX: %f V\r\n",(XAdcPs_GetAdcData(&adcps, XADCPS_CH_VCCAUX)>>4)/4096.0f*3.0f);
	printf("VPVN: %f V\r\n",(XAdcPs_GetAdcData(&adcps, XADCPS_CH_VPVN)>>4)/4096.0f*3.0f);
	printf("VREFP: %f V\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VREFP)>>4)/4096.0f*3.0f);
	printf("VREFN: %f V\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VREFN)>>4)/4096.0f*3.0f);
	printf("VBRAM: %f V\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VBRAM)>>4)/4096.0f*3.0f);
	printf("VCCPINT: %f\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VCCPINT)>>4)/4096.0f*3.0f);
	printf("VCCPAUX: %f\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VCCPAUX)>>4)/4096.0f*3.0f);
	printf("VCCPDRO: %f\r\n", (XAdcPs_GetAdcData(&adcps, XADCPS_CH_VCCPDRO)>>4)/4096.0f*3.0f);
