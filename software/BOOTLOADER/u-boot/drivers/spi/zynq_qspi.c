...
static int zynq_qspi_probe(struct udevice *bus)
{
	...
	ret = gpio_request_by_name(bus, "gpios", 0, &priv->wp_gpio, GPIOD_IS_OUT);
	if(ret) {
		// pr_err("Gpios not found %d\n", ret);
		//no WP gpio
		priv->wp_gpio.dev = 0;
	}
...
static void zynq_qspi_wpset(struct  zynq_qspi_priv *priv, int is_on) {
	// pr_err("Setting WP to %d\n", is_on);
	if(priv->wp_gpio.dev != 0) {
		dm_gpio_set_value(&priv->wp_gpio, is_on);
		// pr_err("Set WP to %d\n", is_on);
	}
}
			     ...
			     const struct spi_mem_op *op)
{
	...
	if(slave->flags & SPI_XFER_WP) {
		flag |= SPI_XFER_WP;
		slave->flags &= ~SPI_XFER_WP;
	}
