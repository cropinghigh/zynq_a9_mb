...
static int mx_write_sr_cr(struct spi_nor *nor, u8 *sr_cr)
{
	...
	nor->spi->flags |= SPI_XFER_WP;

	write_enable(nor);
	
	nor->spi->flags |= SPI_XFER_WP;

	ret = nor->write_reg(nor, SPINOR_OP_WRSR, sr_cr, (nor->flags&SNOR_F_NO_CONF_REG)?1:2);
	if (ret < 0) {
		dev_dbg(nor->dev,
			"error while writing configuration register\n");
		return -EINVAL;
...
static int mx_read_cr(struct spi_nor *nor)
{
	int ret;
	u8 val;
	if(nor->flags&SNOR_F_NO_CONF_REG) {
		return 0x00;
	}
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	if (JEDEC_MFR(info) == SNOR_MFR_MACRONIX) {
		...
		if(JEDEC_ID(info) == 0x2015) {
			nor->flags |= SNOR_F_NO_CONF_REG;
		}
	}
#endif
