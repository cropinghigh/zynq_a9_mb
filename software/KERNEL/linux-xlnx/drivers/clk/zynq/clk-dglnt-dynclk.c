			   ...
			   struct dglnt_dynclk_mode *bestPick)
{
	...
	while (curDiv <= maxDiv && !freq_found) {
		...
		while (curFb <= maxFb && !freq_found) {
			...
			if (curError < bestError) {
				...
				// pr_info("Digilent dynclk: Trying frequency %u(requested %u); curDiv = %u; curFb = %u; curClkDiv = %u", curFreq, freq, curDiv, curFb, curClkDiv);
			}
			if (!curError)
				freq_found = true;
			curFb++;
	...
	pr_info("Digilent dynclk: Requested frequency %u, got %u", freq, bestPick->freq);
	...
	unsigned long rate, unsigned long parent_rate)
{
	...
	pr_info("Digilent dynclk: Requested rate: %lu Parent rate: %lu", rate, parent_rate);
	...
	if (!dglnt_dynclk_find_mode(rate, parent_rate, &clkMode)) {
		pr_info("Digilent dynclk: No mode found");
		return -EINVAL;
	}
